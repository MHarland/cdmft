...
class Reblock:
    ...
    def _transform(self, x):
        result = {b[0]: np.zeros([len(b[1]), len(b[1])], dtype = complex) for b in self.new}
        for bn, b in x.items():
            for i, j in itt.product(range(b.shape[0]), range(b.shape[1])):
                if not ((bn, i, j) in self.rmap.keys()):
                    assert np.allclose(x[bn][i,j], 0), "reblocking omits entry "+bn+str(i)+str(j)
        for old, new in self.rmap.items():
            result[new[0]][new[1], new[2]] = x[old[0]][old[1], old[2]]
        return result

    def _inverse(self, x):
        result = {b[0]: np.zeros([b[1], b[1]], dtype = complex) for b in self.old}
        for bn, b in x.items():
            for i, j in itt.product(range(b.shape[0]), range(b.shape[1])):
    ...
    def _transform_gf(self, g):
        result = BlockGf(name_list = [b[0] for b in self.new], block_list = [GfImFreq(indices = b[1], mesh = g.mesh) for b in self.new])
        for b, i, j in g.all_indices:
            if not( (b, int(i), int(j)) in self.rmap.keys()):
                data = g[b][i,j].data[:,0,0]
                assert np.allclose(np.sum(data), 0), "reblocking omits entry "+b+str(i)+str(j)
        for old, new in self.rmap.items():
            result[new[0]][new[1], new[2]] << g[old[0]][old[1], old[2]]
        return result
        
    def _inverse_gf(self, g):
        result = BlockGf(name_list = [b[0] for b in self.old], block_list = [GfImFreq(indices = b[1], mesh = g.mesh) for b in self.old])
        for b, i, j in g.all_indices:
...
class UnitaryMatrixTransformation:
    ...
    def _transform(self, x):
        prod = {}
        for bn, b in x.items():
            prod[bn] = self.mat[bn].dot(b).dot(np.transpose(self.mat[bn]).conjugate())
        return prod

    def _inverse(self, x):
        prod = {}
        for bn, b in x.items():
            prod[bn] = np.transpose(self.mat[bn]).conjugate().dot(b).dot(self.mat[bn])
        return prod
    ...
    def _transform_gf(self, x):
        ...
        for bn, b in x:
            ...
            for i,j,k,l in itt.product(*[orbs]*4):
                prod[bn][i,l] += self.mat[bn][i,j] * x[bn][j,k] * np.transpose(self.mat[bn])[k,l].conjugate()
        return prod
    ...
    def _inverse_gf(self, x):
        ...
        for bn, b in x:
            ...
            orbs = range(norbs)
            for i,j,k,l in itt.product(*[orbs]*4):
                prod[bn][i,l] += np.transpose(self.mat[bn])[i,j].conjugate() * x[bn][j,k] * self.mat[bn][k,l]
        return prod
