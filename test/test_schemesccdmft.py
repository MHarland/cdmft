...
class TestSchemesCCDMFT(unittest.TestCase):
    ...
    def test_SchemesCCDMFT_init(self):
        ...
        gloc = GLocal(glat_orb_struct, gcluster_orb_struct, r, weights_r, hopping_r, nk, imp_to_lat_r,
                      lat_r_to_cluster, impurity_transformation, r_cavity, r_cluster, impurity_blocknames, [1]*4, 10, 1000)

        #sigmaimp = BlockGf(name_block_generator = [(s, GfImFreq(beta = 10, n_points = 1001, indices = [0])) for s in spins], make_copies=False)
        #sigmaimp = MatsubaraGreensFunction(spins, [2, 2], 10, 1000)
        sigmaimp = MatsubaraGreensFunction(
            [s+'-'+m for s, m in itt.product(spins, momenta)], [1]*4, 10, 1000)
        mu = 1.
