...
class TestMatsubaraGreensFunction(unittest.TestCase):
    ...
    def test_MatsubaraGreensFunction_initialization(self):
        ...
        g3 = BlockGf(name_list=['up'], block_list=[
                     GfImFreq(beta=10, n_points=1000, indices=range(2))])
    ...
    def test_MatsubaraGreensFunction_make_g_tau_real(self):
        ...
        for s, b in g:
            b.data[:, :, :] = (np.random.rand(
                b.data.shape[0], b.data.shape[1], b.data.shape[2]) - .5) * .001
            b.data[:, :, :] += complex(0, 1) * (np.random.rand(
                b.data.shape[0], b.data.shape[1], b.data.shape[2]) - .5) * .001
        for s, b in g:
    ...
    def test_MatsubaraGreensFunction_basic_math(self):
        gtriqs = BlockGf(name_list=['up'], block_list=[
                         GfImFreq(beta=10, n_points=1000, indices=range(2))])
