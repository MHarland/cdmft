...
class TestSetupHypercubic(unittest.TestCase):
    ...
    def test_plot_pade(self):
        ...
        mesh = np.array([w for w in g_pade.mesh.values()])
    ...
    def test_plot_rho_npts_convergence(self):
        ...
        for npts, col in zip(nptss, colors):
            ...
            ax.plot(iwmesh.imag, g['up'].data[wmin:wmax, 0,
                                              0].imag, color=col, label="$"+str(npts)+"$")
        ax.set_xlabel('$\omega$')
    ...
    def test_plot_rho_w_convergence(self):
        ...
        for w, col in zip(ws, colors):
            ...
            wmin, wmax = n0, n0+50
            iwmesh = np.array([iw for iw in g.mesh])[wmin:wmax]
            ax.plot(iwmesh.imag, g['up'].data[wmin:wmax,
                                              0, 0].imag, color=col, label="$"+str(w)+"$")
        ax.set_xlabel('$\omega$')
        ax.set_ylabel('$\\Im G(i\\omega)$')
        ax.set_xlim(left=0)
    ...
    def test_Cycle_run(self):
        sto = Storage("test.h5")
        params = TestDMFTParameters({'verbosity': 0, 'measure_G_l': True, 'measure_G_tau': False,
                                     'measure_pert_order': False, 'n_cycles': 10**4, 'n_warmup_cycles': 5*10**3, 'n_l': 30})
