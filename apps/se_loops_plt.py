...
for fname in sys.argv[1:]:
    ...
    for l, c in zip(range(-n_loops, 0), colors):
        ...
        if index is None:
            b, i, j = [(b, i[0], j[0]) for b, i, j in se.all_indices][orb_nr]
        ...
        y_a = diff[b][i, j].data[n_iw0:n_w_max].imag
    ...
    ax.set_xlim(left=0)
    plt.savefig(fname[:-3]+"_se_loc_imp.pdf")
    print fname[:-3]+"_se_loc_imp.pdf ready"
    plt.cla()
