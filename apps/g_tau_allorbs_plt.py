...
for arch_name in sys.argv[1:]:
    ...
    for loop, alpha in zip([-2,-1], [.2,1.]):
        ...
        g = BlockGf(name_block_generator = [(s, GfImTime(beta = giw.beta, n_points = 3001, indices = [i for i in b.indices])) for s, b in giw])
        for s, b in giw:
            g[s] << InverseFourier(b)
        #inds = [(b, i, j) for b, i, j in g.all_indices if i == j or not only_diag]
        inds = [(b, i, j) for b, i, j in g.all_indices if b == 'XY' and (int(i),int(j)) in [(0,2),(2,0),(3,1),(1,3)]]
        ...
        for (b, i, j), color in zip(inds, colors):
            ...
            if alpha == 1.:
                ax.plot(mesh, g[b].data[:,int(i),int(j)].real, color = color, label = "$"+b+"_{"+str(i)+str(j)+"}$", alpha = alpha)
            else:
                ax.plot(mesh, g[b].data[:,int(i),int(j)].real, color = color, alpha = alpha)
            ax.plot(mesh, g[b].data[:,int(j),int(i)].real, color = color, alpha = alpha)
