...
transform = MatrixTransformation(
    gf_struct_site, transformation, gf_struct_site)
...
for fname in sys.argv[1:]:
    ...
    loops = range(n_loops)
    orbs = [(k, i, j)
            for k, i, j in itt.product(['GM', 'XY'], range(4), range(4))]
    orders = ['N', 'SC', 'AFM', 'FM']
    nc = len(orbs)
    ...
    for l in loops:
        ...
        for orb in orbs:
            ...
            if i == j:
                ntot += n
        sco = (g['XY'][0, 1].density() + g['XY'][1, 0].density() -
               g['XY'][2, 3].density() - g['XY'][3, 2].density()) * .25
        # print g['XY'][0, 1].density(), g['XY'][1, 0].density(), g['XY'][2, 3].density(), g['XY'][3, 2].density()
        ...
        afm = .125 * (gsite["up"][0, 0] + gsite["up"][3, 3] + gsite["dn"][1, 1] + gsite["dn"][2, 2]
                      - gsite["dn"][0, 0] - gsite["dn"][3, 3] - gsite["up"][1, 1] - gsite["up"][2, 2]).density()
        fm = .125 * (gsite["up"][0, 0] + gsite["up"][3, 3] + gsite["up"][1, 1] + gsite["up"][2, 2]
                     - gsite["dn"][0, 0] - gsite["dn"][3, 3] - gsite["dn"][1, 1] - gsite["dn"][2, 2]).density()
        ...
        if verbose and l == loops[-1]:
            ...
            print g['XY'][0, 1].density().real, g['XY'][1, 0].density().real, g['XY'][2, 3].density().real, g['XY'][3, 2].density().real
            print gsite["up"][0, 0].density().real, gsite["up"][3, 3].density().real, gsite["dn"][1, 1].density().real, gsite["dn"][2, 2].density().real
            print gsite["dn"][0, 0].density().real, gsite["dn"][3, 3].density().real, gsite["up"][1, 1].density().real, gsite["up"][2, 2].density().real
