...
from pytriqs.gf import BlockGf, GfImTime, inverse
...
for archive_name in sys.argv[1:]:
    ...
    g = BlockGf(name_block_generator=[(s, GfImTime(indices=[
                i for i in b.indices], beta=gw.mesh.beta, n_points=len(gw.mesh)*2))for s, b in gw], make_copies=False)
    for s, b in gw:
