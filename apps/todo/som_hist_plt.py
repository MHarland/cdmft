...
for archive_name in sys.argv[1:]:
    ...
    if isinstance(gori, GfLegendre):
        ax.plot(mesh, np.log(
            abs(gori.data[:, 0, 0].real)), label="original", marker="+")
        ax.plot(mesh, np.log(
            abs(grec.data[:, 0, 0].real)), ls='--', label="reconstructed", marker='x')
        ...
        ax.plot(mesh[pr[0]:pr[1]], gori.data[pr[0]:pr[1],
                                             0, 0].real, label="original", marker="+")
        ax.plot(mesh[pr[0]:pr[1]], grec.data[pr[0]:pr[1], 0, 0].real,
                ls='--', label="reconstructed", marker='x')
        ax.set_xlabel("$\\tau$")
        ax.set_ylabel("$G(\\tau)$")
