...
class SingleBetheSetup(CycleSetupCommon):
    ...
    def __init__(self, beta, mu, u, t_bethe, n_iw=1025, afm=False):
        ...
        self.h_int = hubbard
        self.gloc = GLocalWithOffdiagonals(
            t_bethe, t_loc, blocknames, blocksizes, beta, n_iw)
        ...
        self.mu = mu
        self.global_moves = {
            "spin-flip": {("up", 0): ("dn", 0), ("dn", 0): ("up", 0)}}
        self.quantum_numbers = [
            hubbard.get_n_tot(), hubbard.get_n_per_spin(up)]
...
class TriangleBetheSetup(CycleSetupCommon):
    ...
    def __init__(self, beta, mu, u, t_triangle, t_bethe, orbital_labels=["E", "A2", "A1"],
                 ...
                 site_transformation=np.array([[1/np.sqrt(3), 1/np.sqrt(3), 1/np.sqrt(3)], [
                                              0, -1/np.sqrt(2), 1/np.sqrt(2)], [-np.sqrt(2./3.), 1/np.sqrt(6), 1/np.sqrt(6)]]),
                 n_iw=1025, afm=False):
        up = "up"
        dn = "dn"
        spins = [up, dn]
        ...
        transf = self.transf = MatrixTransformation(
            gf_struct_site, transfbmat, gf_struct)
        a = t_triangle
        ...
        self.se = SelfEnergy(blocknames, blocksizes, beta, n_iw)
        self.mu = mu
        # {"spin-flip": dict([((s1+"-"+k, 0), (s2+"-"+k, 0)) for k in orbital_labels for s1, s2 in itt.product(spins, spins) if s1 != s2]), "A1A2-flip": dict([((s+"-"+k1, 0), (s+"-"+k2, 0)) for s in spins for k1, k2 in itt.product(xy, xy) if k1 != k2])}
        self.global_moves = {}
        self.quantum_numbers = [
            hubbard.get_n_tot(), hubbard.get_n_per_spin(up)]
...
class TwoOrbitalDimerBetheSetup(CycleSetupCommon):
    def __init__(self, beta, mu, u, j, tc_perp, td_perp, tc0_bethe, tc1_bethe, td0_bethe, td1_bethe, density_density_only=False, orbitals=["c", "d"], symmetric_orbitals=[], n_iw=1025, site_transformation=np.array([[1/np.sqrt(2), 1/np.sqrt(2)], [1/np.sqrt(2), -1/np.sqrt(2)]])):
        ...
        t_loc_site = {bn: t_loc_site for bn, t_loc_site in zip(
            blocknames, [tc_loc_site, td_loc_site, tc_loc_site, td_loc_site])}
        tc_bethe_site = np.array([[tc0_bethe, 0], [0, tc1_bethe]])
        td_bethe_site = np.array([[td0_bethe, 0], [0, td1_bethe]])
        t_bethe_site = {bn: t_bethe_site for bn, t_bethe_site in zip(
            blocknames, [tc_bethe_site, td_bethe_site, tc_bethe_site, td_bethe_site])}
        ...
        self.h_int = KanamoriDimer(
            u, j, spins, orbitals, density_density_only=density_density_only, transf=site_transformation)
        self.g0 = WeissFieldInhomogeneous(blocknames, blocksizes, beta, n_iw)
        self.gloc = GLocalInhomogeneous(
            t_bethe, t_loc, blocknames, blocksizes, beta, n_iw)
        self.se = SelfEnergy(blocknames, blocksizes, beta, n_iw)
        self.mu = mu
        self.global_moves = {}
...
class TwoMixedOrbitalDimerBetheSetup(CycleSetupCommon):
    def __init__(self, beta, mu, u, j, tc_perp, td_perp, tc_bethe, td_bethe, tcd_bethe, density_density_only=False, orbitals=["c", "d"], symmetric_orbitals=[], n_iw=1025, site_transformation=np.array([[1/np.sqrt(2), 1/np.sqrt(2)], [1/np.sqrt(2), -1/np.sqrt(2)]]), fm=False):
        ...
        tx_loc = -1 * tg_loc
        t_loc = {bn: t_loc_site for bn, t_loc_site in zip(
            blocknames, [tg_loc, tx_loc, tg_loc, tx_loc])}
        tg_bethe = np.array([[tc_bethe, tcd_bethe], [tcd_bethe, td_bethe]])
        tx_bethe = -1 * tg_bethe
        t_bethe = {bn: tb for bn, tb in zip(
            blocknames, [tg_bethe, tx_bethe, tg_bethe, tx_bethe])}
        self.h_int = KanamoriMixedOrbitalMomentumDimer(
            u, j, spins, orbs=orbitals, density_density_only=density_density_only, transf=site_transformation, momenta=momenta)
        if not fm:
            self.g0 = WeissFieldInhomogeneous(
                blocknames, blocksizes, beta, n_iw)
            self.gloc = GLocalInhomogeneous(
                t_bethe, t_loc, blocknames, blocksizes, beta, n_iw)
        else:
            self.g0 = WeissFieldInhomogeneousFM(
                blocknames, blocksizes, beta, n_iw)
            self.gloc = GLocalInhomogeneousFM(
                t_bethe, t_loc, blocknames, blocksizes, beta, n_iw)
        self.se = SelfEnergy(blocknames, blocksizes, beta, n_iw)
        self.mu = mu
        self.global_moves = {}
...
class TwoOrbitalMomentumDimerBetheSetup(CycleSetupCommon):
    def __init__(self, beta, mu, u, j, tc_perp, td_perp, tc_bethe, td_bethe, density_density_only=False, orbitals=["c", "d"], symmetric_orbitals=[], n_iw=1025, site_transformation=np.array([[1/np.sqrt(2), 1/np.sqrt(2)], [1/np.sqrt(2), -1/np.sqrt(2)]]), fm=False):
        ...
        sites = range(2)
        momenta = ["G", "X"]
        blocknames = [s+"-"+o+"-"+k for s, o,
                      k in itt.product(spins, orbitals, momenta)]
        blocksizes = [1] * 8
        gf_struct = [[n, range(s)] for n, s in zip(blocknames, blocksizes)]
        blocknames_site = [s+"-"+o for s, o in itt.product(spins, orbitals)]
        blocksizes_site = [2] * 4
        gf_struct_site = [[n, range(s)] for n, s in zip(
            blocknames_site, blocksizes_site)]
        tc_loc_site = np.array([[0, tc_perp, ], [tc_perp, 0]])
        td_loc_site = np.array([[0, td_perp, ], [td_perp, 0]])
        t_loc_site = {bn: t_loc_site for bn, t_loc_site in zip(
            blocknames_site, [tc_loc_site, td_loc_site, tc_loc_site, td_loc_site])}
        site_transformation = {
            bn: site_transformation for bn in blocknames_site}
        self.transf = MatrixTransformation(
            gf_struct_site, site_transformation, gf_struct)
        t_loc = self.transf.transform_matrix(t_loc_site)
        ...
        for bn in blocknames:
            if "-"+orbitals[0] in bn:
                ...
                assert False, "neither " + \
                    orbitals[0]+" nor "+orbitals[1]+" in "+bn
        self.h_int = KanamoriMomentumDimer(
            u, j, spins, orbitals, density_density_only=density_density_only, momenta=momenta, transf=site_transformation)
        if not fm:
            self.g0 = WeissFieldInhomogeneous(
                blocknames, blocksizes, beta, n_iw)
            self.gloc = GLocalInhomogeneous(
        ...
        self.se = SelfEnergy(blocknames, blocksizes, beta, n_iw)
        self.mu = mu
        self.global_moves = {"spin-flip": {(s1+"-"+o+"-"+k, 0): (s2+"-"+o+"-"+k, 0)
                                           for s1, s2, o, k in itt.product(spins, spins, orbitals, momenta) if s1 != s2}}
        self.quantum_numbers = [self.h_int.n_tot(), self.h_int.sz_tot()]
...
class TriangleAIAOBetheSetup(CycleSetupCommon):
    ...
    def __init__(self, beta, mu, u, t_triangle, t_bethe, n_iw=1025, force_real=True,
        ...
        blocknames_paramag = [
            s+"-"+k for s in self.spins for k in momentum_labels]
        gf_struct_paramag = [[n, range(1)] for n in blocknames_paramag]
        self.paramag_to_aiao = MatrixTransformation(
            gf_struct=gf_struct_paramag, gf_struct_new=gf_struct)
        ...
        transf = MatrixTransformation(
            gf_struct_site, site_transformation, gf_struct)
        self.t_loc = transf.transform_matrix(t_loc)
        self.h_int = TriangleSpinOrbitCoupling(
            bn, u, transf=site_transformation)
        self.mu = mu
        self.gloc = GLocalAIAO(t_bethe, self.t_loc,
                               self.blocknames, blocksizes, beta, n_iw)
        self.se = SelfEnergy(self.blocknames, blocksizes, beta, n_iw)
        self.g0 = WeissFieldAIAO(self.blocknames, blocksizes, beta, n_iw)
        self.global_moves = {"spin-flip": dict([((bn, i), (bn, (i+3) % 6)) for i in range(6)]), "reflection": dict(
            [((bn, 1), (bn, 2)), ((bn, 2), (bn, 1)), ((bn, 4), (bn, 5)), ((bn, 5), (bn, 4))])}
    ...
    def set_initial_guess(self, selfenergy, g0, e_in=.15, e_out=-.15, v=.15, transform=True, momentum_labels=None):
        ...
        for (i, j, k), shift in zip(rotation_permutations, excitation_shifts):
            self.add_dynamical_aiao_field(
                e_in * shift, e_out * shift, v, [i, j, k])
    ...
    def add_dynamical_aiao_field(self, e_in, e_out, v, permutation):
        ...
        for s0, i0, s1, i1 in itt.product(spins, sites, spins, sites):
            ...
            se[bn][a0, a1] += np.sum([u[i0, j] * r[j][t, s0].real * v**2 * inverse(iOmega_n - eps[t])
                                      * r[j][t, s1].real * u[i1, j].conjugate() for t, j in itt.product(spins, sites)])
...
class PlaquetteBetheSetup(CycleSetupCommon):
    ...
    def __init__(self, beta, mu, u, tnn_plaquette, tnnn_plaquette, t_bethe,
        ...
        t_loc_per_spin = np.array(
            [[0, a, a, b], [a, 0, b, a], [a, b, 0, a], [b, a, a, 0]])
        t_loc = {up: t_loc_per_spin, dn: t_loc_per_spin}
        t_loc = transf.transform_matrix(t_loc)
        ...
        self.se = SelfEnergy(blocknames, blocksizes, beta, n_iw)
        self.mu = mu
        self.global_moves = {"spin-flip": dict([((s1+"-"+k, 0), (s2+"-"+k, 0)) for k in orbital_labels for s1, s2 in itt.product(
            spins, spins) if s1 != s2]), "XY-flip": dict([((s+"-"+k1, 0), (s+"-"+k2, 0)) for s in spins for k1, k2 in itt.product(xy, xy) if k1 != k2])}
        self.quantum_numbers = [
            hubbard.get_n_tot(), hubbard.get_n_per_spin(up)]
...
class NambuMomentumPlaquette(CycleSetupCommon):
    ...
    def __init__(self, beta, mu, u, tnn_plaquette, tnnn_plaquette, t_bethe=1, n_iw=1025):
        ...
        self.transformation = dict(
            [(s, transformation_matrix) for s in self.spins])
        a, b = tnn_plaquette, tnnn_plaquette
        t_loc = np.array([[0, a, a, b], [a, 0, b, a],
                          [a, b, 0, a], [b, a, a, 0]])
        t_loc = {up: np.array(t_loc), dn: np.array(t_loc)}
        self.reblock_map = {(up, 0, 0): (g, 0, 0), (dn, 0, 0): (g, 1, 1), (up, 1, 1): (x, 0, 0), (dn, 1, 1): (
            x, 1, 1), (up, 2, 2): (y, 0, 0), (dn, 2, 2): (y, 1, 1), (up, 3, 3): (m, 0, 0), (dn, 3, 3): (m, 1, 1)}
        self.mom_transf = MatrixTransformation(
            self.gf_struct_site, self.transformation, self.gf_struct, reblock_map=self.reblock_map)
        t_loc = self.mom_transf.transform_matrix(t_loc)
        self.mu = mu
        self.operators = PlaquetteMomentumNambu(
            u, self.spins, self.momenta, self.transformation)
        self.h_int = self.operators.get_h_int()
        self.gloc = GLocalNambu(
            t_bethe, t_loc, self.momenta, [2]*4, beta, n_iw)
    ...
    def apply_dynamical_sc_field(self, gap):  # TODO is it the gap?
        ...
        for offdiag in [[0, 1], [1, 0]]:
            for n in [n_points, n_points-1]:
                ...
                g[xi].data[inds] = gap * g.mesh.beta * .5
    ...
    def transform_to_nambu(self, g, g_nambu):
        ...
        gf_struct_mom = dict([(s+'-'+k, [0])
                              for s in self.spins for k in self.momenta])
    ...
    def backtransform_g(self, giw):
        ...
        gtarget = BlockGf(name_block_generator=[[s, GfImFreq(
            beta=giw.mesh.beta, indices=range(8), n_points=len(giw.mesh))] for s in ['full']], make_copies=False)
        for i, j, s1, s2, k in itt.product(self.sites, self.sites, self.spins, self.spins, self.momenta):
            gtarget['full'][4*i_s[s1]+i, 4*i_s[s2]+j] << gtarget['full'][4*i_s[s1]+i, 4*i_s[s2] +
                                                                         j] + u.transpose().conjugate()[i, i_k[k]] * giw[k][i_s[s1], i_s[2]] * u[i_k[k], j]
...
class AFMNambuMomentumPlaquette(NambuMomentumPlaquette):
    ...
    def __init__(self, beta, mu, u, tnn_plaquette, tnnn_plaquette, t_bethe=-1, n_iw=1025):
        ...
        if not isinstance(t_bethe, dict):
            t_bethe = {gm: t_bethe *
                       np.identity(4), xy: t_bethe * np.identity(4)}
            for s, m in t_bethe.items():
                t_bethe[s][1, 1] *= -1
                t_bethe[s][3, 3] *= -1
        self.mu = mu
        self.operators = PlaquetteMomentumAFMNambu(
            u, self.spins, self.momenta, self.transformation)
        self.h_int = self.operators.get_h_int()
        self.gloc = GLocalAFMNambu(
            t_bethe, t_loc, self.momenta, [4]*2, beta, n_iw)
    ...
    def apply_dynamical_sc_field(self, gap):
        ...
        for offdiag in [[0, 1], [1, 0]]:
            for n in [n_points, n_points-1]:
                inds = tuple([n] + offdiag)
                g["XY"].data[inds] = gap * g.mesh.beta * .5
        for offdiag in [[2, 3], [3, 2]]:
            for n in [n_points, n_points-1]:
                inds = tuple([n] + offdiag)
                g["XY"].data[inds] = -1 * gap * g.mesh.beta * .5
    ...
    def apply_dynamical_staggered_field(self, gap):
        ...
        for index in indices_up:
            for n in [n_points, n_points-1]:
                b, i, j = index
                self.se[b].data[n, i, j] += gap * self.se.mesh.beta * .5
    ...
    def init_centered_semicirculars(self):
        ...
        for n, b in self.se:
            for i in b.indices:
                i = int(i)
                b[i, i] << (-1)**i * (np.identity(1) *
                                      self.mu - self.gloc.t_loc[n][i, i])
