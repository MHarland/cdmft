...
from pytriqs.gf import SemiCircular
...
class MomentumDimerSetup(CycleSetupCommon):
    ...
    def __init__(self, beta, mu, u, t, n_k, spins=['up', 'dn'], momenta=['G', 'X'], sites=range(2), n_iw=1025):
        ...
        r = [[0.], [-1.], [1.]]
        imp_to_lat_r = [{(s, 0, 0): (s, 0, 0) for s in spins}, {(s, 0, 1): (
            s, 0, 0) for s in spins}, {(s, 1, 0): (s, 0, 0) for s in spins}]
        weights_r = [1.]*3
        ...
        lat_r_to_cluster = {}
        lat_r_to_cluster.update(
            {((0.), (0.), s, 0, 0): (s, 0, 0) for s in spins})
        lat_r_to_cluster.update(
            {((0.), (1.), s, 0, 0): (s, 0, 1) for s in spins})
        lat_r_to_cluster.update(
            {((1.), (0.), s, 0, 0): (s, 1, 0) for s in spins})
        lat_r_to_cluster.update(
            {((1.), (1.), s, 0, 0): (s, 1, 1) for s in spins})
        one = np.eye(1, 1)
        ...
        site_transf_mat = dict(
            [(s, np.sqrt(.5) * np.array([[1, 1], [1, -1]])) for s in spins])
        ...
        self.gloc = GLocal(glat_orb_struct, gcluster_orb_struct, r, weights_r, hopping_r, n_k, imp_to_lat_r,
                           lat_r_to_cluster, impurity_transformation, r_cavity, r_cluster, impurity_blocknames, [1]*4, 10, n_iw)
        for s, b in self.gloc:
            b << SemiCircular(np.abs(t))
        self.se = SelfEnergy(
            [s+'-'+m for s, m in itt.product(spins, momenta)], [1] * 4, beta, n_iw)
        self.g0 = WeissField(tcluster, [s[0]
                                        for s in new_struct], [1] * 4, beta, n_iw)
        # {"spin-flip": dict([((s1+"-"+k, 0), (s2+"-"+k, 0)) for k in momenta for s1, s2 in itt.product(spins, spins) if s1 != s2])}
        self.global_moves = {}
